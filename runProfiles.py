...
def main(argv):
    allDataFile = h5py.File('data/AllDataSets0.h5', 'r')
    smallDataFile = h5py.File('data/GreenlandInBedCoord_V2.h5', 'r')
    os.chdir(argv[1])
    myFiles = []
    ...
    print "starting VX"
    datasetDict = {}
    datasetDict['VX'] = Dataset('VX', smallDataFile)
    print "VX", time.time() - t0
    print "starting VY"
    t0 = time.time()
    datasetDict['VY'] = Dataset('VY', smallDataFile)
    print "VY", time.time() - t0
    print "starting thickness"
    t0 = time.time()
    datasetDict['thickness'] = Dataset('thickness', smallDataFile)
    ...
    for file in myFiles:
        ...
        for shear in myShears:
            print shear

        plt.scatter(shear1X, shear1Y, color='red')
        plt.scatter(shear2X, shear2Y, color='blue')
        plt.show()
        ...
        for x in numTrials:
            ...
            try:
                method1TotalValue, method1StepValue, method1TotalFlux, method1StepFlux = method1(myShears, x, widthList,
                                                                                                 1000, datasetDict,
                                                                                                 integrator)
            except:
                e = sys.exc_info()[0]
            ...
            try:
                method2TotalValue, method2StepValue, method2TotalFlux, method2StepFlux = method2(myShears, x, widthList,
                                                                                                 1000, datasetDict)
            except:
                e = sys.exc_info()[0]
